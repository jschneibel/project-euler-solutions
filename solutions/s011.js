/*
 * Solution to Project Euler problem 11 using JavaScript.
...
for (let row = 0; row < 20; row++) {
  for (let col = 0; col < 20; col++) {
    product = Math.max(product,
      traverseMatrix(matrix, row, col, 0, 1, 4),
      traverseMatrix(matrix, row, col, 0, -1, 4),
      traverseMatrix(matrix, row, col, 1, 0, 4),
      traverseMatrix(matrix, row, col, -1, 0, 4),
      traverseMatrix(matrix, row, col, 1, 1, 4),
      traverseMatrix(matrix, row, col, 1, -1, 4),
      traverseMatrix(matrix, row, col, -1, 1, 4),
      traverseMatrix(matrix, row, col, -1, -1, 4));
  }
...
function traverseMatrix(matrix, row, col, rowStep, colStep, numberOfSteps) {
  let product = 1;

  for (let i = 0; i < numberOfSteps; i++) {
    let newRow = row + i * rowStep;
    let newCol = col + i * colStep;
